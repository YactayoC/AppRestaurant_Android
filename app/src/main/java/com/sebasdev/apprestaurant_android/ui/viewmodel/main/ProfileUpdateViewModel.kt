...
class ProfileUpdateViewModel(private val preferencesDataStore: PreferencesDataStore): ViewModel() {
  ...
  fun onProfileUpdateChange(fullname: String, phone: String, email: String, password: String, direction: String, district: String) {
    ...
    viewModelScope.launch {
      if (_fullname.value != null && _phone.value != null && _email.value != null && _password.value != null && _direction.value != null && _district.value != null) {
        Log.d(
          ...
          "onUpdateUser: " + _fullname.value + " " + _phone.value + " " + _email.value + " " + _password.value + " " + _direction.value + " " + _district.value + " " + _profile.value
        )
        // userUseCase.updateUser()
      }
      if (_password.value == null){
        _message.value = "Porfavor confirme su contraseña"
      }
//      userUseCase.updateUser()
    }
  }
